...
# dont do anything if current appimage is not older than 30 days
N_DAYS_AGO=/tmp/n-days-ago.$$
touch -d "30 days ago" $N_DAYS_AGO
...
RELEASES_JSON="$(curl -sSL "${API_URL}?per_page=500")"
...
# find x86_64 appimage but not a weekly build (thanks claude!)
ASSET_INFO="$(echo "${RELEASES_JSON}" | jq -r '
    [.[] | .assets[] |
     select(.name | test("x86_64.*\\.AppImage$"; "i"))
     | select(.name | test("weekly") | not) ]
    | first
...
# make sure timestamp is current timestamp
touch "${TARGET_PATH}"

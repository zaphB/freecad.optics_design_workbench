...
if [[ ! -e freecad-weekly-appimage.AppImage ]] || [[ "freecad-weekly-appimage.AppImage" -ot "$N_DAYS_AGO" ]]; then
  ...
  DOWNLOAD_URL=$(curl https://api.github.com/repos/FreeCAD/FreeCAD-Bundle/releases | grep 'weekly.*conda-Linux-x86_64-py311.AppImage' | head -2 | grep 'download' | grep -Po '"[^"]+"' | tail -1 | sed 's/\"//g')

...
TODO: optics workbench author fragen ob er mit der Aussage damit happy ist
...
To install a specific version of the workbench python package in just tell pip so: `pip install optics_design_workbench==v1.2.0` will install workbench version v1.2.0.

To install the FreeCAD addon in a specific version requires a few more steps, because the Addon manager will generally install the latest version of the workbench. To manually install a specific workbench version, head to the releases section of the workbench repository TODO LINK EINFUEGEN and download the zipped source of your version of choice. Extract downloaded zip archive to the Mod folder of your FreeCAD installation. Make sure that the folder containing the `init_gui.py` is on the third subfolder level below the Mod directory, e.g. like this:
...
The install the python package in development mode, clone the repository TODO LINK REPOSITORY and install the python module in editable mode using `pip install -e .`. Create a symlink in your FreeCAD's Mod folder pointing to the directory of the cloned directory (If the workbench is not appearing make sure that the folder containing the `init_gui.py` is on the third subfolder level below the Mod directory). With this setup, changes in the cloned repository folder will be effective immediately when restarting FreeCAD as described above.
